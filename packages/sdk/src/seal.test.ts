...
test("open fails (null) on a tampered ciphertext", () => {
  const sender = nacl.box.keyPair();
  const recipient = nacl.box.keyPair();
  const framed = sealToProvider(new TextEncoder().encode("hi"), recipient.publicKey, sender.secretKey);
...
test("sealConfidential FAILS CLOSED on a non-confidential provider", async () => {
  ...
  const att = { publicKey: "x", encryptionPubKey: "y", selfSignature: "" } as unknown as AttestationRecord;
  await assert.rejects(
    () => sealConfidential(new TextEncoder().encode("secret"), att, undefined, { knownGoodCdHashes: [] }),
...
test.skipIf(!existsSync(CONF_FIXTURE))(
  ...
  async () => {
    ...
    const sealed = await sealConfidential(new TextEncoder().encode("private prompt"), att, att.mdaCertChain, {
