      ...
      >
        ...
        {showHalf && (
          <span {...stylex.props(styles.halfStarWrapper)}>
            <Star size={size} {...stylex.props(styles.starEmpty, styles.halfStarBase)} />
            <Star
              size={size}
              fill="currentColor"
        ...
        {Array.from({ length: emptyCount }, (_, i) => (
          <Star key={`empty-${String(i)}`} size={size} {...stylex.props(styles.starEmpty)} />
        ))}
      </div>
...
export function StarRatingInput({
  ...
  const handleStarsMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const index = Math.min(MAX_STARS - 1, Math.max(0, Math.floor((x / rect.width) * MAX_STARS)));
  ...
  return (
    ...
    >
      <div
        ref={starsRef}
        {...stylex.props(styles.stars, styles.starsInput, isDisabled && styles.starsInputDisabled)}
        style={{ "--star-size": `${String(size)}px` } as React.CSSProperties}
...
function StarRatingInputButton({
  ...
  return (
    <button
      ...
      disabled={isDisabled}
      {...pressProps}
      {...stylex.props(styles.starButton, isDisabled && styles.starButtonDisabled)}
    >
      <Star
        size={size}
