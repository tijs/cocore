...
import { animationDuration, animationTimingFunction } from "../theme/animations.stylex";
...
function AvatarContent({
  ...
  const showFallback = !src || (hasCheckedImage && (imageError || !imageLoaded));

  return (
    <>
      {src && !imageError && (
        <AvatarImageWithState key={src} src={src} alt={alt} onStateChange={handleStateChange} />
      )}
      {showFallback && (
        <div
          {...stylex.props(
...
export function Avatar({ style, alt = "", src, fallback, size: sizeProp, ...props }: AvatarProps) {
  ...
  return (
    ...
    >
      <AvatarContent key={src} src={src} alt={alt} fallback={fallback} size={size} />
    </div>
  );
}
...
export function AvatarButton({
  ...
  return (
    <AriaButton data-avatar-button {...buttonProps} {...stylex.props(styles.buttonWrapper, style)}>
