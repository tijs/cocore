...
import { estimatedRowHeights, useListBoxItemStyles } from "../theme/useListBoxItemStyles";
...
export interface ListBoxProps<T extends object> extends StyleXComponentProps<AriaListBoxProps<T>> {
  size?: Size;
  items?: Iterable<T>;
...
export function ListBox<T extends object>({
  ...
  const handleSelectionChange = (keys: Parameters<NonNullable<typeof onSelectionChange>>[0]) => {
    trigger("selection");
...
function ListBoxCheckbox({ id = "", ...props }: CheckboxProps) {
  ...
  return (
    <Checkbox
      {...props}
      isSelected={Boolean(listboxItemState?.selectionManager.selectedKeys.has(id))}
      onChange={() => listboxItemState?.selectionManager.select(id)}
    />
  );
}

export function ListBoxItem({ style, children, prefix, suffix, ...props }: ListBoxItemProps) {
          ...
          {variant === "checkbox" && (
            <div {...stylex.props(listBoxItemStyles.addon)}>
              <ListBoxCheckbox
                ...
                id={(props.id as string) || (props.value as { id?: string } | undefined)?.id}
                onPress={(e) => e.continuePropagation()}
              />
            </div>
          )}
          {prefix != null && <div {...stylex.props(listBoxItemStyles.addon)}>{prefix}</div>}
          <div {...stylex.props(listBoxItemStyles.label)}>{children}</div>
          {suffix != null && <div {...stylex.props(listBoxItemStyles.addon)}>{suffix}</div>}
...
export interface ListBoxSectionProps<T extends object> extends StyleXComponentProps<
  AriaListBoxSectionProps<T>
> {
  children: React.ReactNode;
}

export function ListBoxSection<T extends object>({ style, ...props }: ListBoxSectionProps<T>) {
...
export function ListBoxSectionHeader({ style, ...props }: ListBoxSectionHeaderProps) {
  ...
  return (
    <Header
      {...props}
      data-size={size}
      {...stylex.props(typeramp.sublabel, styles.sectionLabel, ui.textDim, style)}
    />
  );
}
