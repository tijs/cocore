...
import { fontFamily, fontSize, fontWeight, lineHeight } from "../theme/typography.stylex";
...
const styles = stylex.create({
  ...
  cardHeader: {
    // Mobile: stack the action below the title/description so a wide
    // header action (e.g. a multi-segment SegmentedControl) doesn't
    // squeeze the title/description into a sliver of a column. At `sm`
    // and up we restore the side-by-side title|action layout.
    gridTemplate: {
    ...
    gridTemplateColumns: {
      ...
      [breakpoints.sm]: "minmax(0, 1fr) auto",
    },
  ...
  cardHeaderAction: {
    ...
    display: "flex",
    flexWrap: "wrap",
    // Left-align when stacked under the title on mobile; right-align
    // when it sits beside the title at `sm` and up.
...
export interface CardProps extends StyleXComponentProps<React.ComponentProps<"div">> {
  size?: Size;
}
...
export const Card = ({ style, size: sizeProp, ...props }: CardProps) => {
  ...
  return (
    <SizeContext value={size}>
      <div {...props} data-card-size={size} {...stylex.props(styles.card, ui.bg, ui.text, style)} />
    </SizeContext>
  );
};

export interface CardHeaderProps extends StyleXComponentProps<React.ComponentProps<"div">> {
  hasBorder?: boolean;
}
...
export interface CardTitleProps extends StyleXComponentProps<React.ComponentProps<"h2">> {}

export const CardTitle = ({ style, ...props }: CardTitleProps) => {
  return <div {...props} {...stylex.props(styles.cardTitle, style)} />;
};

export interface CardDescriptionProps extends StyleXComponentProps<React.ComponentProps<"p">> {}
...
export interface CardHeaderActionProps extends StyleXComponentProps<React.ComponentProps<"div">> {}

export const CardHeaderAction = ({ style, ...props }: CardHeaderActionProps) => {
  return <div {...props} {...stylex.props(styles.cardHeaderAction, style)} />;
};
export interface CardBodyProps extends StyleXComponentProps<React.ComponentProps<"div">> {}

export const CardBody = ({ style, ...props }: CardBodyProps) => {
  return <div {...props} {...stylex.props(styles.cardSection, styles.cardBody, style)} />;
};

export interface CardFooterProps extends StyleXComponentProps<React.ComponentProps<"div">> {}

export const CardFooter = ({ style, ...props }: CardFooterProps) => {
  return <div {...props} {...stylex.props(styles.cardSection, styles.cardFooter, style)} />;
};

export interface CardImageProps extends StyleXComponentProps<React.ComponentProps<"img">> {
  aspectRatio?: number;
