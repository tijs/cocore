...
export interface CalendarProps<T extends DateValue>
  extends StyleXComponentProps<AriaCalendarProps<T>>, Pick<CalendarGridProps, "weekdayStyle"> {
  errorMessage?: string;
}
...
function MonthDropdown() {
  ...
  const monthsInYear = state.focusedDate.calendar.getMonthsInYear(state.focusedDate);
  ...
  return (
    <Select
      ...
      style={styles.select}
      onChange={(key) => {
        const month = months.find((item) => key && item.id === getNumericKey(key));

        if (month) {
          state.setFocusedDate(month.date as Parameters<typeof state.setFocusedDate>[0]);
        }
      }}
    >
...
function YearDropdown() {
  ...
  return (
    <Select
      ...
      style={styles.select}
      onChange={(key) => {
        const year = years.find((item) => key && item.id === getNumericKey(key));

        if (year) {
          state.setFocusedDate(year.date as Parameters<typeof state.setFocusedDate>[0]);
        }
      }}
    >
      {(item) => <SelectItem>{item.formatted}</SelectItem>}
  ...
  return (
    ...
    >
      <header {...stylex.props(styles.header)}>
        <IconButton variant="secondary" slot="previous" aria-label="Previous month">
          <ChevronLeft />
        </IconButton>
      ...
      <Flex align="start" gap="2xl">
        {monthsVisible.map((month) => (
          ...
          >
            <CalendarGridHeader>
              {(day) => (
                <CalendarHeaderCell {...stylex.props(calendarStyles.headerCell)}>
                  {day}
                </CalendarHeaderCell>
              )}
            </CalendarGridHeader>
            <CalendarGridBody>
              {(date) => <CalendarCell date={date} {...stylex.props(calendarStyles.cell)} />}
            </CalendarGridBody>
          </CalendarGrid>
        ))}
      </Flex>
