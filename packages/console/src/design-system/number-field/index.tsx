...
import { horizontalSpace, size as sizeSpace } from "../theme/semantic-spacing.stylex";
...
function NumberInputWrapper({ style, onClick, children }: NumberInputWrapperProps) {
  ...
  return (
    <div {...stylex.props(styles.wrapper, style)} {...mergeProps(lockProps, { onClick })}>
      {children}
      {isLocked &&
        createPortal(
          <div {...cursorProps}>
...
function NumberFieldContent({
  ...
  const content = (
    <>
      ...
      */}
      <NumberInputWrapper style={inputStyles.wrapper} onClick={() => inputRef.current?.focus()}>
        {!hideStepper && (
          <Button
            slot="decrement"
            {...stylex.props(styles.button, styles.buttonLeft, ui.borderInteractive, ui.bgAction)}
          >
            <Minus />
          </Button>
        )}
        {prefix != null && <div {...stylex.props(inputStyles.addon)}>{prefix}</div>}
        <Input
        ...
        {!hideStepper && (
          <Button
            slot="increment"
            {...stylex.props(styles.button, styles.buttonRight, ui.borderInteractive, ui.bgAction)}
          >
            <Plus />
          </Button>
        )}
      ...
      <FieldErrorMessage style={inputStyles.errorMessage}>{errorMessage}</FieldErrorMessage>
    </>
  );
