...
import { FileTrigger as AriaFileTrigger, Button, DropZone } from "react-aria-components";
...
async function getFiles(items: Array<DropItem>): Promise<Array<File>> {
  return Promise.all(items.filter((item) => item.kind === "file").map((item) => item.getFile()));
}
...
interface FileDropZoneProps
  extends Omit<AriaFileTriggerProps, "className" | "style">, Pick<DropZoneProps, "isDisabled"> {
  ...
  return (
    <DropZone
      ...
      onDrop={(e) => {
        void getFiles(e.items).then((files) => {
          ...
          const matched = files.filter((file) =>
            acceptedFileTypes.some((type) => fileMatchesAcceptedType(file, type)),
          );
          onAddFiles?.(matched);
        });
      }}
      getDropOperation={(types) => {
        ...
        return hasAcceptedDropType(acceptedFileTypes, types) ? "copy" : "cancel";
      }}
    >
      {({ isDropTarget }) => {
        if (isDropTarget) {
...
export const FileDropDefaultTrigger = ({
  ...
  return (
    <Button {...stylex.props(styles.defaultTrigger, ui.bgGhost, style)} {...props}>
      {children}
    </Button>
  );
};
