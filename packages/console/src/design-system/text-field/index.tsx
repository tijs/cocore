...
import { TextField as AriaTextField, Input, InputContext } from "react-aria-components";
  ...
  const content = (
    <>
      ...
      */}
      ...
      <div {...stylex.props(inputStyles.wrapper)} onClick={() => inputRef.current?.focus()}>
        {prefix != null && <div {...stylex.props(inputStyles.addon)}>{prefix}</div>}
        <Input
          {...stylex.props(inputStyles.input, inputStyle)}
          ref={inputRef}
          placeholder={placeholder}
          data-focus-always-visible
        />
        {isPasswordInput && (
          <PasswordToggle type={type} setType={setType} style={inputStyles.addon} />
        )}
        <SuffixIcon
          suffix={suffix}
    ...
    <>
      {label && (
        <Label style={inputStyles.label}>
          {label} {isRequired ? <span {...stylex.props(inputStyles.required)}>*</span> : null}
        </Label>
      )}
...
export interface TextFieldProps
  ...
  suffix?: React.ReactNode;
  /**
   * Additional StyleX styles merged onto the native input element.
   */
  inputStyle?: stylex.StyleXStyles;
}
...
export function TextField({
  ...
  const [type, setType] = useState<TextFieldProps["type"]>(props.type || "text");
  const inputStyles = useInputStyles({
    size,
    variant,
    labelVariant,
