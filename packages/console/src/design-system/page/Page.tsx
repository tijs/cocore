...
import { breakpoints, containerBreakpoints } from "../theme/media-queries.stylex";
...
const largeHeaderStyles = stylex.create({
  header: {
    // Mobile-first: stack the title and actions vertically so a wide
    // actions cluster (Export / Clean up / Pair, segmented controls,
    // etc.) wraps under the title instead of overflowing the viewport.
    // The page description is hidden below `sm` (see sharedStyles),
    // so on mobile we only lay out title + actions. At `sm` and up we
    // restore the side-by-side grid (with the optional icon column).
    gridTemplateAreas: {
      default: `
        'title'
        'actions'
    ...
    gridTemplateColumns: {
      ...
      ":has([data-page-icon])": "auto minmax(0, 1fr)",
      [breakpoints.sm]: {
        default: "minmax(0, 1fr) auto",
        ":has([data-page-icon])": "auto minmax(0, 1fr) auto",
      },
    },
    rowGap: gap["md"],
    marginBottom: {
...
const sharedStyles = stylex.create({
  ...
  largeActions: {
    ...
    // Wrap a crowded action cluster onto multiple lines on narrow
    // screens rather than overflowing horizontally. Left-aligned when
    // stacked under the title on mobile; right-aligned beside it at sm+.
    flexWrap: "wrap",
    justifyContent: { default: "flex-start", [breakpoints.sm]: "flex-end" },
    minHeight: sizeSpace["3xl"],
  },
  icon: {
...
export interface PageRootProps extends StyleXComponentProps<React.ComponentProps<"div">> {
  /**
 ...
 */
export const PageRoot = ({ style, variant = "large", ...props }: PageRootProps) => {
  const rootStyles = variant === "small" ? smallRootStyles.root : largeRootStyles.root;

  return (
    <PageContext value={variant}>
      <div {...props} data-page-variant={variant} {...stylex.props(rootStyles, style)} />
    </PageContext>
  );
};

export interface PageHeaderProps extends StyleXComponentProps<React.ComponentProps<"div">> {}
...
export const PageHeader = ({ style, ...props }: PageHeaderProps) => {
  ...
  if (isSmall) {
    return <Flex align="center" gap="xl" {...props} style={[smallHeaderStyles.header, style]} />;
  }

  return <div {...props} {...stylex.props(largeHeaderStyles.header, style)} />;
};

export interface PageIconProps extends StyleXComponentProps<React.ComponentProps<"div">> {}
...
export const PageIcon = ({ style, ...props }: PageIconProps) => {
  return <div data-page-icon {...props} {...stylex.props(sharedStyles.icon, style)} />;
};

export interface PageTitleProps extends StyleXComponentProps<React.ComponentProps<"h1">> {
  /** Page title content. */
...
export const PageTitle = ({ style, children, ...props }: PageTitleProps) => {
  const variant = usePageContext();
  const isSmall = variant === "small";
  const titleStyles = isSmall ? sharedStyles.smallTitle : sharedStyles.largeTitle;

  return (
    <Text
      size={isSmall ? { default: "xl", sm: "2xl" } : { default: "2xl", sm: "3xl" }}
      weight="semibold"
      {...props}
...
export interface PageDescriptionProps extends StyleXComponentProps<React.ComponentProps<"p">> {
  /** Description content. */
  children: React.ReactNode;
}
 ...
 * Only used with variant="large".
 */
export const PageDescription = ({ style, children, ...props }: PageDescriptionProps) => {
  return (
    <Text
      size="sm"
      variant="secondary"
...
export interface PageActionsProps extends StyleXComponentProps<React.ComponentProps<"div">> {}
...
export const PageActions = ({ style, ...props }: PageActionsProps) => {
  const variant = usePageContext();
  const isSmall = variant === "small";
  const actionsStyles = isSmall ? sharedStyles.smallActions : sharedStyles.largeActions;
 ...
 */
export const PageStickyHeader = ({ children, style }: PageStickyHeaderProps) => {
  const variant = usePageContext();
  const isSmall = variant === "small";
  ...
  return (
    <div {...stylex.props(stickyBaseStyles.displayContents)}>
      <div ref={sentinelRef} {...stylex.props(sentinelStyles)} />
      <div
        ref={headerRef}
        data-sticky-header
        {...stylex.props(wrapperStyles, isStuck && stickyBaseStyles.stickyWrapperStuck, style)}
      >
        {isStuck && (
...
export const PageStickyFooter = ({ children }: PageStickyFooterProps) => {
  ...
  return (
    <>
      <div ref={sentinelRef} {...stylex.props(stickyFooterBaseStyles.sentinel)} />
      <div
        ref={footerRef}
        data-sticky-footer
        {...stylex.props(
