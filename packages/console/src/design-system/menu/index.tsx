...
export interface MenuProps<T extends object>
  extends
    ...
    Pick<
      PopoverProps,
      "shouldCloseOnInteractOutside" | "shouldFlip" | "shouldUpdatePosition" | "placement"
    > {
    ...
    <SizeContext value={size}>
      <MenuTrigger defaultOpen={defaultOpen} isOpen={isOpen} onOpenChange={handleOpenChange}>
        ...
        >
          ...
          <AriaMenu {...props} onAction={handleAction} {...stylex.props(styles.menu)} />
          {Boolean(footer) && (
            <>
              <ListBoxSeparator />
...
export interface MenuSectionProps<T extends object> extends StyleXComponentProps<
  AriaMenuSectionProps<T>
> {
  children: React.ReactNode;
}

export function MenuSection<T extends object>({ style, ...props }: MenuSectionProps<T>) {
  return <AriaMenuSection {...props} {...stylex.props(style)} />;
}

export interface MenuItemProps extends StyleXComponentProps<Omit<AriaMenuItemProps, "children">> {
  children: React.ReactNode;
    ...
    >
      {({ isSelected, hasSubmenu }) => (
        <div {...stylex.props(menuItemStyles.inner)}>
          {prefix != null && <div {...stylex.props(menuItemStyles.addon)}>{prefix}</div>}
          <div {...stylex.props(menuItemStyles.label)}>{children}</div>
          {suffix != null && <div {...stylex.props(menuItemStyles.addon)}>{suffix}</div>}
          {isSelected && (
