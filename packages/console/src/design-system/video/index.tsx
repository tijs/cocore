...
import { horizontalSpace, size, verticalSpace } from "../theme/semantic-spacing.stylex";
...
type VideoTrackKind = "captions" | "chapters" | "descriptions" | "metadata" | "subtitles";
type VideoMediaTag =
  | "cloudflare-video"
  | "dash-video"
  | "hls-video"
...
function getMediaTag(src?: string, mediaType?: VideoMediaTag): VideoMediaTag {
  ...
  if (
    ...
    (normalizedSrc.includes("customer-") && normalizedSrc.includes(".cloudflarestream.com"))
  ) {
  ...
  if (normalizedSrc.includes(".m3u8") || normalizedSrc.includes("getvideoplaylist")) {
    return "hls-video";
  }

  if (normalizedSrc.includes("jwplayer.com") || normalizedSrc.includes("jwplatform.com")) {
  ...
  if (normalizedSrc.includes("player.vimeo.com") || normalizedSrc.includes("vimeo.com")) {
    return "vimeo-video";
  }
...
function useVideoSubtitles({ rootRef, mediaRef, src, subtitleTracks }: UseVideoSubtitlesOptions) {
  ...
  const defaultSubtitleTrackIndex = subtitleTracks.findIndex((track) => track.default);
  const selectedSubtitleTrackIndex = defaultSubtitleTrackIndex >= 0 ? defaultSubtitleTrackIndex : 0;
  ...
  useEffect(() => {
    const captionsButton = rootRef.current?.querySelector("media-captions-button");
  ...
  useEffect(() => {
    ...
    controller.addEventListener("mediatogglesubtitlesrequest", stopNativeSubtitleToggle, true);
    controller.addEventListener("mediashowsubtitlesrequest", showCustomSubtitles, true);
    controller.addEventListener("mediadisablesubtitlesrequest", hideCustomSubtitles, true);

    return () => {
      controller.removeEventListener("mediatogglesubtitlesrequest", stopNativeSubtitleToggle, true);
      controller.removeEventListener("mediashowsubtitlesrequest", showCustomSubtitles, true);
      controller.removeEventListener("mediadisablesubtitlesrequest", hideCustomSubtitles, true);
    };
  ...
  useEffect(() => {
    ...
    const syncSubtitleState = () => {
      const availableSubtitleTracks = Array.from(textTracks).filter(isSubtitleTrack);
      const selectedTrack =
        captionsEnabled && selectedSubtitleTrackIndex < availableSubtitleTracks.length
      ...
      setActiveSubtitleText(selectedTrack ? getActiveCueText(selectedTrack) : "");
    };
...
export function Video({
  ...
  const { activeSubtitleText, controlsVisible, hasSubtitleTracks } = useVideoSubtitles({
    rootRef,
    mediaRef,
    src,
    subtitleTracks,
  ...
  const resolvedSubtitleOffset =
    subtitleOffset ??
    (controlsVisible ? CAPTION_CONTROLS_VISIBLE_OFFSET : CAPTION_CONTROLS_HIDDEN_OFFSET);

  return (
    <div ref={rootRef} {...stylex.props(styles.root, rounded && styles.rounded, ui.bgDim, style)}>
      <MediaController {...stylex.props(styles.controller, styles.controllerTheme)}>
