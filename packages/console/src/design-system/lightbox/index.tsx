...
export function Lightbox({
  ...
  useLayoutEffect(() => {
    ...
    const runAnimation = () => {
      ...
      const outKeyframes = isNext
        ? [{ transform: "translateX(0)" }, { transform: `translateX(-${String(slideDistance)}px)` }]
        : [{ transform: "translateX(0)" }, { transform: `translateX(${String(slideDistance)}px)` }];
      const inKeyframes = isNext
        ? [{ transform: `translateX(${String(slideDistance)}px)` }, { transform: "translateX(0)" }]
        : [
  ...
  return (
    <DialogTrigger isOpen={isOpen} onOpenChange={onOpenChange}>
      ...
      <ModalOverlay isDismissable {...stylex.props(styles.overlay, ui.overlay, style)}>
        <div {...stylex.props(styles.backdrop)} onClick={() => onOpenChange(false)} aria-hidden />
        {/* oxlint-disable-next-line jsx_a11y/click-events-have-key-events, jsx_a11y/no-static-element-interactions */}
        <div {...stylex.props(styles.closeButton)} onClick={(e) => e.stopPropagation()}>
          <IconButton variant="tertiary" size="lg" label="Close" slot="close">
            <X size={24} />
          </IconButton>
        </div>
        <Modal {...stylex.props(styles.modal)} onClick={(e) => e.stopPropagation()}>
          <AriaDialog {...stylex.props(styles.dialog)} aria-label={alt}>
            {hasMultiple ? (
                ...
                >
                  {previousIndex === null ? (
                    ...
                    <>
                      <div
                        ref={outgoingRef}
                        {...stylex.props(styles.imageLayer, styles.imageLayerOutgoing)}
                      >
                        <img
                      ...
                      <div
                        ref={incomingRef}
                        {...stylex.props(styles.imageLayer, styles.imageLayerIncoming)}
                      >
                        <img
                          src={currentImage}
                          alt={`${alt} ${String(currentIndex + 1)}`}
              ...
              >
                <img src={currentImage} alt={alt} {...stylex.props(styles.image)} />
              </div>
            )}
          </AriaDialog>
        </Modal>
