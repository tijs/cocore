...
export function ProgressBar({
  ...
  return (
    <SizeContext value={size}>
      <AriaProgressBar {...props} data-size={size} {...stylex.props(styles.wrapper, style)}>
        {({ percentage, valueText, isIndeterminate }) => (
          <>
            {label && <Label style={styles.label}>{label}</Label>}
            {showValueLabel && <span {...stylex.props(styles.valueLabel)}>{valueText}</span>}
            <div {...stylex.props(styles.bar)}>
              {isIndeterminate ? (
                <div {...stylex.props(styles.indeterminateFill)} />
              ) : (
                <div {...stylex.props(styles.fill, styles.progress(percentage ?? 0))} />
              )}
            </div>
          </>
        )}
