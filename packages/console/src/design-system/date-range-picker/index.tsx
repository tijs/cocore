...
function DateRangePickerContent<T extends DateValue>({
  ...
  const content = (
    <>
      <Group data-size={size} {...stylex.props(inputStyles.wrapper, styles.group)}>
        <DateInput slot="start" {...stylex.props(inputStyles.input)}>
          {(segment) => <DateSegment segment={segment} {...stylex.props(styles.segment)} />}
        </DateInput>
        <div aria-hidden="true" {...stylex.props(styles.separator)}>
          -
        </div>
        <DateInput slot="end" {...stylex.props(inputStyles.input, styles.lastInput)}>
          {(segment) => <DateSegment segment={segment} {...stylex.props(styles.segment)} />}
        </DateInput>
        <SuffixIcon
          suffix={
            <IconButton size="sm" aria-label="Open date picker" variant="tertiary">
              <CalendarIcon />
            </IconButton>
          }
  ...
  return (
    <>
      ...
      <AriaPopover {...stylex.props(popoverStyles.wrapper, popoverStyles.animation)}>
        <Dialog {...stylex.props(styles.popoverContent)}>
          <RangeCalendar weekdayStyle={weekdayStyle} visibleDuration={visibleDuration} />
        </Dialog>
      </AriaPopover>
    </>
  );
