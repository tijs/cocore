...
/** Run `fn` against a fresh in-memory AccountStore behind the account
 *  router on an ephemeral port. */
function withAccounts(fn: (base: string) => Promise<void>): Promise<void> {
  const accountStore = new AccountStore(":memory:");
  return withAppviewServer(buildAccountRouter(accountStore, APPVIEW_DID), fn);
}
...
describe("account.* over the AppView", () => {
  ...
  it("scopes keys to the owner — Bob can't see, revoke, or delete Alice's key", async () => {
    await withAccounts(async (base) => {
      ...
      const bobList = (await (
        await fetch(`${base}/xrpc/${N}.listApiKeys`, authed(BOB))
      ).json()) as {
        keys: unknown[];
      };
